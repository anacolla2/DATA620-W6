...
I'll add some color to the nodes to differenciate between event and person.
...
While it may be tempting to assume relationships because of clusters of women in
the graph, but what the graph is really depicting is each woman's relationship
to an event not relationship between the women; a two mode bipartite graph. To
infer about relationships the graph must be changed into a one-mode network.
...
the women. Ill remove the women's relationship with themselves (diagonal values)
because it'll overpower my visualizations.
...
Chapter 5 of () defines a function for whats called the Island Method, which
will basically isolate the connections by weight, creating "islands" of strongly
connected networks.
...
## Extracting the top relationships
...
With the analysis completed I can infer that Evelyn, Brenda and Katherina are
central and important in this social club.
...
women have these *island* connections. They have a little social circle within
the club. We can also infer that Event 8 was a hub within the events network,
alot of the same members attended along with the events E8 was tied to.
